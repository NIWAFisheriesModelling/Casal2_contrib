...
ggplot(quantile_selectivity_df, aes(x = bin)) +
  geom_ribbon(aes(ymax  = low, ymin = upp, alpha = 0.5, col = label, fill = label), lwd=0) +
  geom_line(aes(y = mid, col = label, group = label), size =2, alpha = 1) +
  facet_wrap(~label) +
...
quantile_ssb_df$years[quantile_ssb_df$years == "initialisation_phase_1"] = 1971

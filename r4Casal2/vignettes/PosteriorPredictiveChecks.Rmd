...
if(FALSE)
  cat("!include simulated_reports.csl2", file = file.path(config_dir, "config.csl2"), append = T)
## run Casal2 -s
...
bioplt = plot_predictive_dist(sim_data = sim_chatTANbiomass, 
                              obs = data.frame(obs = mpd$chatTANbiomass$Values$observed, 
                                               year = mpd$chatTANbiomass$Values$year), lab = "", plot_type = "violin")
...
normalised_resids = ggplot(mpd$chatTANbiomass$Values, aes(sample = normalised_residuals)) +
  ...
  labs(x = "Theoretical", y = "Sample")
## Plot fits obs
fits = r4Casal2::plot.relative_index(model = mpd, report_label = "chatTANbiomass") + ggtitle("")
##
fits
```

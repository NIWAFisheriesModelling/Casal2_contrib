...
DHARMaResLF = createDHARMa(simulatedResponse = sim_vals$sim_obs$sim_chatTANage, observedResponse = as.numeric(mean_obs_bin), 
  fittedPredictedResponse = NULL, integerResponse = F)

plot(DHARMaResbio, quantreg = F)
plot(DHARMaResLF, quantreg = F)
```
...
rownames(sim_data) = mpd$chatTANage$Values$year
pppAFplt = plot_predictive_dist(sim_data = sim_data, 
          obs = data.frame(obs = mean_obs_bin, year = mpd$chatTANage$Values$year),
